...
void World::interpolate(float alpha) {
	...
	mWorldView.setCenter(alpha * currentView + (1.f - alpha) * previousView);
...
void World::saveCurrentState() {
	...
	currentView = mWorldView.getCenter();
...
void World::buildScene() {
	...
	mPickupVelocity = mMasterSpacecraft->getPickupVelocity(); addEnemies(); mWorldClock.restart();
