...
namespace GUI {
    ...
    Button::Button(const FontHolder &fonts): mCallback(), mButtonBackground(sf::Vector2f(400.f, 100.f)), mText("", fonts.get(Fonts::Main), 65u), mIsToggle(false) {
    ...
    void Button::alterButton(const sf::Color &color, float rectSize, int charSize) {
        ...
        mButtonBackground.setSize(sf::Vector2f(400.f + rectSize, 100.f + rectSize)); mText.setCharacterSize(65u + charSize);
